...
router.get('/filter', (req, res) => {
    ...
    let filterLetter = req.query.name;
    ...
    if(filterLetter) {
        matchingFriends = matchingFriends.filter(friend => friend.name == filterLetter);
    }
    ...
    if (matchingFriends.length > 0) {
        ...
        res.status(404).json({error: "No friends matching gender "+filterGender+filterLetter})
    }  
})
...
router.get('/info', (req, res) => {
    ...
    let acceptHead = req.headers['accept'];
    console.log(userAgent, contentType, acceptHead);

    // Modify this response to just return info on the user-agent, content-type and accept headers
    res.send(userAgent, contentType, acceptHead);  
})
...
router.put('/:id', (req, res) => {
    ...
    updatedFriend? res.status(201).json({result: 'Updated friend with ID ' + friendId, data: updatedFriend})
       :res.status(404).json({result: `COULD NOT UPDATE ${friendId}`})
})
